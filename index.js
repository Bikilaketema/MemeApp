function theme() {
  let themes = ['index.css','light.css','green.css','darkelegance.css'];
  ...
  link.setAttribute('href', themes[index]);
}
...
function clearAll() {
  ...
  let riddleContainer = document.querySelector('.riddle');
...
function showMeme() {
  ...
  let memeUrl = getRandomData('memes');
  let memeContainer = document.querySelector('.meme');
...
function showJoke() {
  ...
  clearAll();
  jokeContainer.appendChild(newP);
}
...
function showQuote() {
  ...
  quoteContainer.appendChild(newA);
}
...
function showRiddle() {
  ...
  newA.textContent = randomRiddle.answer;
  clearAll();
  newA.hidden = true;
  newA.setAttribute('id', 'answer')
  riddleContainer.appendChild(newR);
  riddleContainer.appendChild(newA);
}
...
function revealAnswers() {
  ...
  if (riddleContainer.childNodes.length == 0) {
    alert('Please get a riddle first!')
  } else if (answer.hidden == true) {
    answer.hidden = false;
  } else if ( answer.hidden == false && riddleContainer.childNodes.length == 2) {
    alert('The answer is already revealed for you!')
  }
}
